...
class FruitsDataset(JointsDataset):
    ...
    def _load_coco_keypoint_annotation_kernel(self, index):
        ...
        record = [{
            ...
            'center': center,
            'scale': scale,
            'joints_xyv': res,  # joints_xyv [self.num_joints, k==3, 5]
            'filename': '',
            'imgnum': 0,
        }]
    ...
    def get_nmsed(self, kp_preds, all_boxes, img_path):
        ...
        for img in kpts.keys():
            ...
            for n_p in img_kpts:
                ...
                kpt_score = 0
                valid_num = 0
                for n_jt in range(0, self.max_joint_num):
            ...
            keep = oks_nms([img_kpts[i] for i in range(len(img_kpts))], oks_thre, sigmas = 0.025 * np.ones(self.max_joint_num))
